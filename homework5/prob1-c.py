...
# set objective
m.setObjective(
    theta_x16 + theta_x17 + theta_x18 + theta_x19 + theta_x26 + theta_x27 + theta_x28 + theta_x29 + theta_x36 + theta_x37 + theta_x38 + theta_x39 + theta_x46 + theta_x47 + theta_x48 + theta_x49 + theta_x56 + theta_x57 + theta_x58 + theta_x59,
    GRB.MINIMIZE
)

# add constraints
m.addConstr(
    541.0*x16 + 386.0*x17 + 25.0*x18 + 1512.0*x19 + 234.0*x26 + 899.0*x27 + 103.0*x28 + 1256.0*x29 + 543.0*x36 + 257.0*x37 + 1653.0*x38 + 1085.0*x39 + 1785.0*x46 + 227.0*x47 + 1670.0*x48 + 823.0*x49 + 490.0*x56 + 1233.0*x57 + 1242.0*x58 + 1841.0*x59 <= 4245880
)
...
costCoef = {"x16": 541.0,"x17": 386.0,"x18": 25.0,"x19": 1512.0,"x26": 234.0,"x27": 899.0,"x28": 103.0,"x29": 1256.0,"x36": 543.0,"x37": 257.0,"x38": 1653.0,"x39": 1085.0,"x46": 1785.0,"x47": 227.0,"x48": 1670.0,"x49": 823.0,"x56": 490.0,"x57": 1233.0,"x58": 1242.0,"x59": 1841.0}
totalCost = 0
for v in m.getVars():
    print(v.varName, v.x, "\n")
    if v.varName in list(costCoef.keys()):
        totalCost += costCoef[v.varName] * v.x
